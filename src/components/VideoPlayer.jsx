...
export default function VideoPlayer({ video }) {
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        // استخدم مدة الفيديو من بيانات السيرفر إن وُجدت، وإلا اعتمد على metadata
        const serverDuration = (video && Number(video.duration)) || 0;
        if (serverDuration > 0) {
        ...
        // Do not autoplay on manifest parsed — user requested no automatic play
        // apply requested quality immediately if set
        try {
          if (currentQuality) {
            applyQualityToHls(hls, currentQuality);
            // lock automatic level switching to avoid aggressive auto quality changes
      ...
      hls.on(Hls.Events.ERROR, (event, data) => {
        try {
          // hls error (silenced in production)
          ...
          if (data && data.type === Hls.ErrorTypes.NETWORK_ERROR && data.details && data.details.indexOf('fragLoad') === 0) {
            ...
            if (url) {
              ...
              if (retries <= 4) {
                ...
                setTimeout(() => { try { hls.startLoad(); } catch (e) { } }, backoff);
                return;
              }
            }
          }

          // Media errors: attempt media recovery first, limited retries
          if (data && data.type === Hls.ErrorTypes.MEDIA_ERROR) {
            mediaErrorRetriesRef.current = (mediaErrorRetriesRef.current || 0) + 1;
            if (mediaErrorRetriesRef.current <= 3) {
              try { hls.recoverMediaError(); } catch (e) { }
              // small delay to let decoder stabilize
              setTimeout(() => { try { if (hls) hls.startLoad(); } catch (e) { } }, 500);
              return;
            }
            // if recovery fails, try a full startLoad then show error
            try { hls.startLoad(); } catch (e) { }
            setError('حدث خطأ في فك ترميز الوسائط');
            return;
          }

          // Generic network error: retry a few times globally
        ...
        } catch (err) { /* suppressed logging in production build */ }
      });

      // Avoid aggressive auto switching: do not auto-change quality on frag load.
      hls.on(Hls.Events.LEVEL_SWITCHED, () => {
        // keep load going and configuration stable
        try { hls.startLoad(); } catch (e) { }
      });
    } else {
      // Native HLS (Safari)
      ...
      videoRef.current.addEventListener('error', () => setError('Playback failed'));
    }

    // ensure dimensions applied when hls or video change
    try { updateVideoSizing(); } catch (e) { }
      ...
      try {
        ...
        if (b && b.length) {
          ...
          if (dur > 0) {
            ...
            if (Math.abs(pct - (bufferedPercent || 0)) > 1) setBufferedPercent(pct);
          }
        }
      } catch (e) { }
    };
    ...
    const saveInt = setInterval(() => {
      try {
        if (videoRef.current && !isNaN(videoRef.current.currentTime)) {
          localStorage.setItem(`video-pos-${video._id}`, String(videoRef.current.currentTime));
        }
      } catch (e) { }
    }, 2000);
    ...
    const handleContainerClick = (e) => {
      ...
      if (!videoRef.current?.paused && !isTouchDevice) {
    ...
    const handleTouchStart = (e) => {
      ...
      const containerWidth = videoRef.current?.parentElement?.offsetWidth || window.innerWidth;
      ...
      // reset/extend hide timeout while interacting
      if (touchHideTimeoutRef.current) clearTimeout(touchHideTimeoutRef.current);
      touchHideTimeoutRef.current = setTimeout(() => {
        if (!longPressActiveRef.current && !qualityMenuOpen && !speedMenuOpen && !videoRef.current?.paused) {
    ...
    const handleTouchEnd = (e) => {
      ...
      // schedule hide after interaction
      if (touchHideTimeoutRef.current) clearTimeout(touchHideTimeoutRef.current);
      touchHideTimeoutRef.current = setTimeout(() => {
        if (!qualityMenuOpen && !speedMenuOpen && !videoRef.current?.paused) {
          setShowTouchControls(false);
          setShowControls(false);
        }
      }, 2500);
      ...
      // إذا لم يكن نقرًا مزدوجًا ولم يكن ضغطًا مطولًا وكانت الحركة صغيرة
      if (!doubleTapHandledRef.current && touchDuration < 500 && deltaX < 15) {
        togglePlayPause();
      }

      // reset double-tap flag for next interactions
      doubleTapHandledRef.current = false;
    ...
    return () => {
      ...
      if (touchControlsTimeout.current) clearTimeout(touchControlsTimeout.current);
      if (tapTimeout.current) clearTimeout(tapTimeout.current);
      if (container) {
        container.removeEventListener('click', handleContainerClick);
        try { container.removeEventListener('touchstart', handleTouchStart); } catch (e) {}
        try { container.removeEventListener('touchmove', handleTouchMove); } catch (e) {}
        try { container.removeEventListener('touchend', handleTouchEnd); } catch (e) {}
      }
      videoRef.current?.removeEventListener('timeupdate', updateTime);
    };
  }, [video, currentQuality, shouldInit]);
        ...
        try { showActionFeedback(nv === 0 ? Icons.VolumeOff : Icons.VolumeLow, `الصوت ${Math.round(nv*100)}%`); } catch (e) {}
          ...
          try {
            ...
            if (v.volume > 0) {
              ...
              setVolume(0);
              v.volume = 0;
              try { showActionFeedback(Icons.VolumeOff, 'كتم'); } catch (e) {}
            } else {
              const restore = (prevVolumeRef.current != null && prevVolumeRef.current > 0) ? prevVolumeRef.current : 1;
              setVolume(restore);
              v.volume = restore;
              try { showActionFeedback(Icons.VolumeHigh, `الصوت ${Math.round(restore*100)}%`); } catch (e) {}
            }
          } catch (err) { setVolume(v.volume === 0 ? 1 : 0); }
      } else if (e.key === '>' || e.key === '.') {
  ...
  useEffect(() => {
    ...
    return () => {
      try { v.removeEventListener('play', onPlay); } catch (e) { }
      try { v.removeEventListener('pause', onPause); } catch (e) { }
    };
  }, [videoRef.current]);
  ...
  useEffect(() => {
    ...
    const onEsc = (e) => { if (e.key === 'Escape') { setQualityMenuOpen(false); setSpeedMenuOpen(false); setSettingsMenuOpen(false); } };
  ...
  const safePlay = () => {
    try {
      const p = videoRef.current && videoRef.current.play && videoRef.current.play();
  ...
  useEffect(() => {
    ...
    try {
      ...
      try { if (hls && currentQuality) hls.autoLevelEnabled = false; } catch (e) { }
    } catch (e) { }
  ...
  const isProgressRtl = () => {
    ...
    } catch (e) { return document && document.dir === 'rtl'; }
  };
  ...
  const rateStep = 0.05;
  const increaseRate = () => setPlaybackRate((r) => Math.min(3, Math.round((r + rateStep) * 100) / 100));
  const decreaseRate = () => setPlaybackRate((r) => Math.max(0.25, Math.round((r - rateStep) * 100) / 100));
  ...
  const Icons = {
    ...
    Replay10: () => (
      <svg className="w-5 h-5 sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M8.5 9.5L5.5 12L8.5 14.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        <path d="M9.5 7.5A6 6 0 0 1 17 12" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" fill="none" />
      </svg>
    ),
    Pause: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    Forward10: () => (
      <svg className="w-5 h-5 sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.4" fill="none" />
        <path d="M15.5 9.5L18.5 12L15.5 14.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        <path d="M14.5 7.5A6 6 0 0 0 7 12" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" fill="none" />
      </svg>
    ),
    Settings: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 15.5C13.93 15.5 15.5 13.93 15.5 12C15.5 10.07 13.93 8.5 12 8.5C10.07 8.5 8.5 10.07 8.5 12C8.5 13.93 10.07 15.5 12 15.5Z" fill="currentColor" />
    ...
    VolumeOff: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M19 12C19 12.94 18.8 13.82 18.46 14.64L19.97 16.15C20.62 14.91 21 13.5 21 12C21 7.72 18.01 4.14 14 3.23V5.29C16.89 6.15 19 8.83 19 12Z" fill="currentColor" />
        <path d="M4.27 3L3 4.27L7.73 9H3V15H7L12 20V13.27L16.25 17.52C15.58 18.04 14.83 18.46 14 18.7V20.77C15.38 20.45 16.63 19.82 17.68 18.93L19.73 21L21 19.73L12 10.73L4.27 3Z" fill="currentColor" />
      </svg>
    ),
    VolumeLow: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    Quality: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M19 4H5C3.89 4 3 4.9 3 6V18C3 19.1 3.89 20 5 20H19C20.1 20 21 19.1 21 18V6C21 4.9 20.11 4 19 4ZM19 18H5V6H19V18Z" fill="currentColor" />
    ...
    Speed: () => (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M20.38 8.57L13.92 2.2C13.72 2.07 13.56 2 13.3 2H6.5C5.67 2 5 2.67 5 3.5V20.5C5 21.33 5.67 22 6.5 22H17.5C18.33 22 19 21.33 19 20.5V9.3C19 9.04 18.93 8.78 18.7 8.58L20.38 8.57Z" fill="currentColor" />
        <path d="M12 17.5C10.07 17.5 8.5 15.93 8.5 14C8.5 12.07 10.07 10.5 12 10.5C13.93 10.5 15.5 12.07 15.5 14C15.5 15.93 13.93 17.5 12 17.5Z" fill="currentColor" />
      </svg>
    ),
    ChevronDown: () => (
    ...
    PauseCircle: () => (
      <svg className="w-8 h-8 sm:w-10 sm:h-10" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9 16H11V8H9V16ZM13 8V16H15V8H13ZM12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.59 20 4 16.41 4 12C4 7.59 7.59 4 12 4C16.41 4 20 7.59 20 12C20 16.41 16.41 20 12 20Z" fill="currentColor" />
      </svg>
    ),
  };
    ...
    <div className="space-y-2 relative">
      ...
      >
        <video
          ...
          poster={video?.poster || video?.thumbnail?.lqip || video?.thumbnail?.small || ''}
          playsInline
          preload="metadata"
        ...
        {loading && (
          <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/50">
            <div className="flex flex-col items-center gap-2">
              <svg className="w-12 h-12 text-white animate-spin" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {isDownloading && downloadProgress != null && (
          <div className="absolute left-1/2 -translate-x-1/2 bottom-4 z-40 pointer-events-none w-2/5">
            <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden shadow-inner">
              <div className="h-full bg-cyan-500 transition-all" style={{ width: `${downloadProgress}%` }} />
            </div>
          </div>
        )}
        {error && (
      ...
      <div
        className={`absolute left-0 right-0 bottom-16 px-2 transition-opacity duration-200 ${((showControls || showTouchControls) ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}`}
        onMouseEnter={() => { setShowControls(true); clearHideControls(); }}
        onMouseLeave={() => { scheduleHideControls(3000); }}
        onFocus={() => { setShowControls(true); clearHideControls(); }}
        onBlur={() => { scheduleHideControls(3000); }}
      >
        <div className="flex items-center gap-3">
          <div className="text-white/90 font-medium text-sm">
            <span className="bg-black/30 px-2 py-1 rounded-md">{formatTime(currentTime)}</span>
          ...
          <div className="flex-1">
            <div
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === 'ArrowRight') {
                  e.preventDefault();
                  const nt = Math.min(duration, (videoRef.current.currentTime || 0) + 5);
                  videoRef.current.currentTime = nt; setCurrentTime(nt);
                } else if (e.key === 'ArrowLeft') {
                  e.preventDefault();
                  const nt = Math.max(0, (videoRef.current.currentTime || 0) - 5);
                  videoRef.current.currentTime = nt; setCurrentTime(nt);
                } else if (e.key === 'Home') {
                  e.preventDefault(); videoRef.current.currentTime = 0; setCurrentTime(0);
                } else if (e.key === 'End') {
                  e.preventDefault(); videoRef.current.currentTime = duration; setCurrentTime(duration);
                }
              }}
              ref={progressRef}
              className={`relative cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 h-2 bg-gradient-to-r from-gray-800/60 to-gray-800/30 rounded-full shadow-inner`}
              onClick={(e) => { e.stopPropagation(); handleSeek(e); if (videoRef.current?.paused) safePlay(); }}
              onMouseDown={(e) => {
                e.stopPropagation();
                handlePointerSeekStart(e);
              }}
              onPointerDown={(e) => { e.stopPropagation(); handlePointerSeekStart(e); }}
            ...
            >
              ...
              <div
                ...
                style={(() => {
                  ...
                  return rtl ? { right: 0, width: w, zIndex: 1 } : { left: 0, width: w, zIndex: 1 };
                })()}
              />
              ...
              <div
                ...
                style={(() => {
                  ...
                  if (rtl) return { right: pct, transform: 'translate(50%, -50%)', zIndex: 2, width: isTouchDevice ? '1.1rem' : '0.9rem', height: isTouchDevice ? '1.1rem' : '0.9rem' };
                  return { left: pct, transform: 'translate(-50%, -50%)', zIndex: 2, width: isTouchDevice ? '1.1rem' : '0.9rem', height: isTouchDevice ? '1.1rem' : '0.9rem' };
                })()}
                aria-hidden
              />
            </div>
          ...
          <div className="flex items-center gap-2 w-auto sm:w-auto justify-end">
            <button onClick={(e) => { e.stopPropagation(); toggleFullscreen(); }} className="flex items-center justify-center rounded-full bg-gradient-to-br from-gray-900/80 to-black/80 text-white min-w-[36px] h-8 sm:min-w-[44px] sm:h-11 sm:w-10 shadow-lg" aria-label={isFullscreen ? 'خروج من ملء الشاشة' : 'ملء الشاشة'}>
      ...
      <div
        className={`absolute left-0 right-0 bottom-4 px-2 transition-opacity duration-200 ${((showControls || showTouchControls) ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}`}
        onMouseEnter={() => { setShowControls(true); clearHideControls(); }}
        onMouseLeave={() => { scheduleHideControls(3000); }}
        onFocus={() => { setShowControls(true); clearHideControls(); }}
        onBlur={() => { scheduleHideControls(3000); }}
        ...
        <div className="flex flex-row items-center justify-between gap-2 sm:gap-3">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap w-full sm:w-auto justify-start">
            <button
              ...
              className="flex items-center justify-center rounded-full bg-gradient-to-br from-gray-900 to-black min-w-[44px] h-9 sm:min-w-[56px] sm:h-12 sm:w-14 text-white shadow-2xl active:scale-95"
              ...
              onClick={(e) => {
                ...
                try {
                  if (volume > 0) {
                    ...
                    setVolume(prevVolumeRef.current != null ? prevVolumeRef.current : 1);
                  }
                } catch (err) { setVolume(volume === 0 ? 1 : 0); }
              }}
              className="flex items-center justify-center rounded-full bg-gradient-to-br from-gray-900/80 to-black/80 min-w-[36px] h-8 sm:min-w-[44px] sm:h-11 sm:w-10 text-white shadow active:scale-95"
            ...
            <div className="relative hidden sm:block" ref={qualityRef} onClick={(e) => e.stopPropagation()}>
              <button
                onClick={(e) => { e.stopPropagation(); setQualityMenuOpen((s) => !s); setSpeedMenuOpen(false); }}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-br from-gray-900/90 to-black/90 text-white px-2 py-1 shadow-lg border border-white/10 min-w-[36px] h-8 sm:min-w-[44px] sm:h-10"
                aria-expanded={qualityMenuOpen}
                aria-label="اختيار الجودة"
              >
                <Icons.Quality />
                <span className="font-medium">{currentQuality ? `${currentQuality}p` : 'الجودة'}</span>
                <Icons.ChevronDown />
              </button>
            ...
            <div className="relative hidden sm:block" ref={speedRef} onClick={(e) => e.stopPropagation()}>
              <button
                onClick={(e) => { e.stopPropagation(); setSpeedMenuOpen((s) => !s); setQualityMenuOpen(false); }}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-br from-gray-900/90 to-black/90 text-white px-2 py-1 shadow-lg border border-white/10 min-w-[36px] h-8 sm:min-w-[44px] sm:h-10"
                aria-expanded={speedMenuOpen}
                aria-label="اختيار السرعة"
              >
                <Icons.Speed />
                <span className="font-medium">{typeof playbackRate === 'number' ? playbackRate.toFixed(2) : playbackRate}x</span>
                <Icons.ChevronDown />
              </button>
              {speedMenuOpen && (
                <div className="absolute bottom-12 right-0 bg-gradient-to-b from-gray-900 to-black border border-white/10 rounded-2xl shadow-2xl w-40 sm:w-56 p-2 sm:p-3 backdrop-blur-lg z-50">
                  <div className="flex items-center flex-col w-full">
                    <div className="flex items-center justify-center gap-2 sm:gap-3 w-full mb-2 sm:mb-3">
                      <button onClick={(e) => { e.stopPropagation(); adjustRate(-rateStep); }} className="bg-black/60 text-white rounded-full w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center text-lg">−</button>
                      <div className="text-lg sm:text-2xl font-semibold text-white">{typeof playbackRate === 'number' ? playbackRate.toFixed(2) : playbackRate}x</div>
                      <button onClick={(e) => { e.stopPropagation(); adjustRate(rateStep); }} className="bg-black/60 text-white rounded-full w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center text-lg">+</button>
                    </div>
            ...
            <div className="relative sm:hidden" ref={settingsRef} onClick={(e) => e.stopPropagation()}>
              <button onClick={(e) => { e.stopPropagation(); setSettingsMenuOpen((s) => !s); setQualityMenuOpen(false); setSpeedMenuOpen(false); }} className="flex items-center justify-center rounded-xl bg-gradient-to-br from-gray-900/90 to-black/90 text-white px-2 py-1 shadow-lg border border-white/10 min-w-[36px] h-8">
                <Icons.Settings />
              </button>
              {settingsMenuOpen && (
                <div className="absolute bottom-12 right-0 bg-gradient-to-b from-gray-900 to-black border border-white/10 rounded-xl shadow-2xl py-2 z-50 w-56 backdrop-blur-lg">
                  ...
                  {video && video.qualities && [...video.qualities].sort((a,b)=>parseInt(a.quality)-parseInt(b.quality)).map(q=> (
                    <button key={q.quality} onClick={(e)=>{e.stopPropagation(); handleQualityChangePersist(q.quality); setSettingsMenuOpen(false);}} className={`w-full text-right px-5 py-2 text-sm ${String(q.quality)===String(currentQuality)?'bg-gradient-to-r from-red-600 to-red-800 text-white font-semibold':'text-white/80'}`}>
                      {q.quality}p
                    </button>
                  ))}
                  <div className="px-3 py-2 text-sm text-white/80">السرعة</div>
                  <div className="px-3 py-2">
                    <input type="range" min="0.25" max="3" step="0.05" value={playbackRate} onChange={(e)=>{ const v = Math.round(parseFloat(e.target.value)*100)/100; setPlaybackRate(v); showRateFeedback(v);}} className="w-full accent-red-600" />
                  </div>
                  <div className="px-3 py-2">
                    <button onClick={(e)=>{ e.stopPropagation(); const d = document.querySelector('[aria-label="تحميل الفيديو"]'); if(d) d.click(); setSettingsMenuOpen(false); }} className="w-full text-left px-4 py-2 text-sm bg-black/20 rounded">تحميل</button>
                  </div>
                </div>
              )}
            </div>
