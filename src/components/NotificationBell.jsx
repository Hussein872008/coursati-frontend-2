...
const DEFAULT_ICON = (
  <div className="w-12 h-8 bg-gray-700/70 rounded-md flex items-center justify-center text-white">م</div>
);
...
const NotificationBell = () => {
  ...
  const fetch = async () => {
    try {
      ...
      if (newest && (!latestRef.current || newest._id !== latestRef.current)) {
        ...
        try { showBrowserNotification(newest); } catch (e) {}
      }
    } catch (err) {}
  };
  ...
  const handleMarkRead = async (id) => {
    try {
      ...
      setNotifications((prev) => prev.map((p) => (p._id === id ? { ...p, isRead: true } : p)));
    } catch (e) {}
  };
  ...
  const showBrowserNotification = async (n) => {
    ...
    try {
      ...
      const subtitleParts = [n.chapterTitle, n.instructorTitle, n.materialTitle].filter(Boolean).join(' - ');
      const body = subtitleParts || '';
      const icon = n.thumbnailUrl || undefined;
      const tag = `notif-${n._id}`;
      const notif = new Notification(n.title, { body, icon, tag, data: { chapterId: n.chapterId, lectureId: n.lectureId, videoId: n.videoId } });
      notif.onclick = (ev) => {
        try { ev.preventDefault(); } catch (e) {}
        try { window.focus(); } catch (e) {}
        try {
          // mark notification as read in backend and update UI
          try { handleMarkRead(n._id); } catch (e) {}
          const qp = new URLSearchParams();
          if (n.lectureId) qp.set('lecture', n.lectureId);
          if (n.videoId) qp.set('video', n.videoId);
          const qs = qp.toString();
  ...
  return (
    <div className="relative">
      <button onClick={handleOpen} className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-white relative">
        <BellIcon className="w-5 h-5" />
        {unreadCount > 0 && <span className="absolute -top-1 -left-1 w-3 h-3 bg-red-500 rounded-full" />}
      </button>
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <div className="font-semibold text-white mb-4 text-lg text-center">الإشعارات</div>
            {notifications.length === 0 && <div className="text-sm text-white/60 text-center">لا توجد إشعارات</div>}
            {notifications.map((n) => (
              <div
                key={n._id}
              ...
              >
                <div className="w-14 h-14 bg-gray-700 rounded-md overflow-hidden flex-shrink-0">
                  {n.thumbnailUrl ? <img src={n.thumbnailUrl} alt="thumb" className="w-full h-full object-cover" /> : DEFAULT_ICON}
                </div>
      ...
      {toast && (
        <div className="fixed left-4 bottom-6 z-50 bg-gray-900/95 text-white rounded-lg p-3 shadow-lg flex gap-3 items-start">
          <div className="w-12 h-12 rounded-md overflow-hidden bg-gray-700">
            {toast.thumbnailUrl ? <img src={toast.thumbnailUrl} className="w-full h-full object-cover" /> : DEFAULT_ICON}
          </div>
          <div>
