              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="flex flex-col">
                    {remainingPrefix && (
                      <div className="text-[11px] text-white/60">{remainingPrefix}</div>
                    )}
                  </div>
                </div>
              </button>
              ...
              >
                <div className="p-3 text-right">
                  <div className="mb-2">
                    ...
                    <div className="text-lg font-semibold text-white truncate" title={user?.name || ''}>
                      {user?.name || 'المستخدم'}
                    </div>
                    {remainingPrefix && (
                      <div className="text-[12px] text-white/60 mt-1">{remainingPrefix}</div>
                    )}
                  </div>
                  {/* Extra details */}
                  <div className="mt-3 text-right text-xs text-white/60">
                    {user?.code && (
                      <div className="flex items-center justify-between gap-2">
                        <div className="truncate">كود المستخدم: <span className="text-white">{user.code}</span></div>
                      </div>
                    )}

                    {user?.createdAt && (
                      <div className="mt-1">انضم: <span className="text-white">{new Date(user.createdAt).toLocaleDateString()}</span></div>
                    )}

                    {user?.subscriptionExpires && (
                      <div className="mt-1">ينتهي: <span className="text-white">{new Date(user.subscriptionExpires).toLocaleString()}</span></div>
                    )}
                  </div>
