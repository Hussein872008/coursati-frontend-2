          ...
          .map((instructor) => {
            ...
            filteredInstructor.chapters = instructor.chapters
              .map((chapter) => {
                ...
                // تصفية المحاضرات
                filteredChapter.lectures = chapter.lectures.filter((lecture) =>
                  lecture.title.toLowerCase().includes(query),
                );

                return filteredChapter.lectures.length > 0
                  ? filteredChapter
                  : null;
              })
              .filter(Boolean);

            return filteredInstructor.chapters.length > 0
              ? filteredInstructor
              : null;
          })
          .filter(Boolean);

        return filteredMaterial.instructors.length > 0
          ? filteredMaterial
          : null;
      })
      .filter(Boolean);
                        ...
                        {expandedItems[chapter._id] && (
                          <div className="ml-4 pl-3 space-y-1">
                            {chapter.lectures?.map((lecture) => (
                              ...
                              >
                                <svg
                                  className="w-4 h-4 ml-2 text-gray-400 group-hover:text-blue-500"
                                  fill="currentColor"
                                  viewBox="0 0 20 20"
                                >
                                  <path
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {searchInTree.length > 0 ? (
          <div className="space-y-2">{searchInTree.map(renderMaterial)}</div>
        ) : (
