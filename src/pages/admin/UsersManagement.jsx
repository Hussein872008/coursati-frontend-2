...
const UsersManagement = () => {
  ...
  const [hideRenewSection, setHideRenewSection] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!isPhoneValid(formData.phone)) {
        toast.error("رقم التليفون غير صالح - يجب أن يكون 11 رقمًا ويبدأ بـ 01", {
          position: "top-right",
          autoClose: 3000,
        });
        return;
      ...
      if (newUser && newUser.code) {
        ...
        const expiryText =
          newUser.subscriptionType === "permanent"
            ? "دائم"
            : newUser.subscriptionExpires
              ? new Date(newUser.subscriptionExpires).toLocaleString("ar-EG")
              : "—";
  ...
  const renewSubscription = async (userId, type) => {
    try {
      ...
      toast.success(
        `تم تجديد الاشتراك ل ${getSubscriptionLabel(type)} بنجاح ✅`,
        {
          position: "top-right",
          autoClose: 3000,
        },
      );
      // hide the renew panel now that subscription was updated
      setHideRenewSection(true);
  ...
  const handleSaveEdit = async (id) => {
    try {
      ...
      if (selectedUser && selectedUser._id === id && updated)
  ...
  const resetDeviceForUser = async (userId) => {
    try {
      ...
      toast.success("تم إعادة تعيين الجهاز للمستخدم بنجاح ✅", {
        position: "top-right",
        autoClose: 2500,
      });
      await loadUsers();
      ...
      toast.error(err.response?.data?.message || "فشل إعادة التعيين ❌", {
        position: "top-right",
        autoClose: 3000,
      });
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-gray-800/95 border border-white/10 rounded-xl p-6 shadow-lg">
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              <button
                onClick={cancelDelete}
                className="px-4 py-2 bg-white/6 text-white rounded-lg hover:bg-white/10 transition-colors"
              >
                إلغاء
              </button>
              <button
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white drop-shadow-sm tracking-tight">
          إدارة المستخدمين
        </h1>
        <button
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-white/70 mt-1">
                أدخل اسم المستخدم ورقم التليفون. الكود يُنشأ تلقائيًا بعد
                الإنشاء.
              </p>
            </div>
            <button
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="permanent" className="bg-gray-800">
                  دائم
                </option>
              </select>
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="hour" className="bg-gray-800">
                  ساعة
                </option>
            ...
            <div className="max-h-[500px] overflow-y-auto custom-scrollbar">
              <table dir="rtl" className="min-w-full table-fixed">
                ...
                <thead className="bg-white/5 border-b border-white/10 sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 text-right align-middle text-sm font-semibold text-white/80 w-48">
                      المعرف
                    </th>
                    <th className="px-4 py-3 text-right align-middle text-sm font-semibold text-white/80 w-40">
                      الاسم
                    </th>
                    <th className="px-4 py-3 text-right align-middle text-sm font-semibold text-white/80 w-36">
                      الكود
                    </th>
                    <th className="px-4 py-3 text-right align-middle text-sm font-semibold text-white/80 w-36">
                      مدة الاشتراك
                    ...
                    <th className="px-4 py-3 text-right align-middle text-sm font-semibold text-white/80 w-56">
                      تاريخ الإنشاء
                    </th>
                    <th className="px-4 py-3 text-center align-middle text-sm font-semibold text-white/80 w-44">
                      الإجراءات
                    </th>
                  </tr>
                </thead>
                      ...
                      <td className="px-4 py-3 align-middle text-sm text-white/80 truncate whitespace-nowrap text-right">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1 bg-white/6 rounded hover:bg-white/10"
                            aria-label="نسخ كود المستخدم"
                          >
                            {copiedId === user.code ? (
      ...
      {showModal && selectedUser && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div
            ...
            className="relative w-full max-w-3xl mx-auto bg-gradient-to-b from-gray-800/95 to-gray-900/95 border border-white/10 rounded-2xl shadow-2xl p-6 animate-fadeIn overflow-y-auto max-h-[90vh] custom-scrollbar"
          >
            {/* Close button */}
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                <h3 className="text-2xl font-bold text-white flex items-center gap-3">
                  ...
                  تفاصيل المستخدم
                  <span className="text-sm ml-2 px-3 py-1 bg-emerald-600/20 text-emerald-200 rounded-full">
                    {getSubscriptionLabel(selectedUser.subscriptionType)}
                  </span>
                </h3>
                <p className="text-sm text-white/70 mt-2">
                  بيانات المستخدم، سجل المشاهدات والاشتراكات، والإجراءات الممكن
                  تنفيذها
                </p>
              </div>
              <button
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-sm font-mono text-white/90 truncate p-2 bg-white/5 rounded mt-1">
                  {selectedUser._id}
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <div className="font-mono font-semibold text-white/90 p-2 bg-white/5 rounded flex-1">
                    {selectedUser.code}
                  </div>
                  <button
                    onClick={() => copyCode(selectedUser.code)}
                    className="p-2 bg-white/6 rounded-lg hover:bg-white/10 text-white transition-colors"
                    title="نسخ الكود"
                    aria-label="نسخ الكود"
                  >
                    {copiedId === selectedUser.code ? (
                      <CheckIcon className="w-4 h-4 text-emerald-400" />
                    ) : (
                      <DocumentDuplicateIcon className="w-4 h-4 text-white/90" />
                    )}
                ...
                <div className="text-sm text-white/90 mt-1">
                  {editUserId === selectedUser._id ? (
                    <input
                      ...
                      className="px-3 py-2 rounded bg-white/5 text-white w-full border border-white/10 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                    />
                  ) : (
              ...
              <div>
                ...
                <div className="text-sm text-white/90 p-2 bg-white/5 rounded mt-1">
                  {selectedUser.subscriptionType === "permanent"
                    ? "دائم"
              ...
              <div>
                ...
                <div className="text-sm text-white/90 p-2 bg-white/5 rounded mt-1">
                  {selectedUser.createdAt
                    ? new Date(selectedUser.createdAt).toLocaleString("ar-EG")
                    : "-"}
                </div>
              </div>
              <div>
                ...
                <div className="text-sm text-white/90 p-2 bg-white/5 rounded mt-1 flex items-center justify-between">
                  <div>
                    {selectedUser.canDownloadVideos ? (
                      <span className="text-emerald-300">مسموح بالتحميل</span>
                    ) : (
                      <span className="text-rose-300">محظور التحميل</span>
                    )}
                  </div>
                  <div>
                    <button
                      onClick={async () => {
                        try {
                          ...
                          toast.success(
                            newVal
                              ? "تم تفعيل تحميل الفيديو لهذا المستخدم"
                              : "تم تعطيل تحميل الفيديو لهذا المستخدم",
                            { position: "top-right", autoClose: 2500 },
                          );
                        } catch (err) {
                          toast.error("فشل تحديث إذن التحميل", {
                            position: "top-right",
                            autoClose: 3000,
                          });
                        }
                    ...
                    <div>
                      {selectedUser.deviceId ? (
                        <span className="text-emerald-300">
                          مسجل بالفعل على متصفح
                        </span>
                      ) : (
                        <span className="text-amber-300">
                          لم يسجل بعد (أول دخول متاح)
                        </span>
                      )}
                    </div>
                    <div>
                  ...
                  {showResetConfirm && (
                    <div className="mt-3 p-3 bg-white/6 border border-white/10 rounded-lg flex items-center justify-between gap-4">
                      <div className="text-sm text-white/80">
                        هل أنت متأكد من إعادة تعيين جهاز هذا المستخدم؟ سيُلغى
                        الوصول من المتصفح الحالي فوراً.
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={async () => {
            ...
            {(() => {
              ...
              return isExpired && !hideRenewSection ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                    {subscriptionOptions.map((option) => (
                      <button
                        key={option.value}
                        onClick={() =>
                          renewSubscription(selectedUser._id, option.value)
                        }
                        className={`relative p-3 rounded-lg border transition-all duration-300 group ${
            ...
            <div className="mt-6">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <div className="ml-0 sm:ml-auto flex items-center gap-2 w-full sm:w-auto">
                  <button
                    onClick={() => loadUserHistory(selectedUser._id)}
                    className="w-full sm:w-auto p-2 bg-white/6 rounded-lg hover:bg-white/10 text-white transition-colors"
                    title="تحديث السجل"
              ...
              <div className="mt-3 bg-white/5 p-4 rounded-lg max-h-60 overflow-y-auto custom-scrollbar">
                {loadingHistory ? (
                  ...
                  <div className="text-white/70 text-center p-4">
                    لا يوجد سجل عرض حالياً. اضغط "تحديث السجل" لتحميله.
                  </div>
                ) : historyTab === "views" ? (
                  <div className="space-y-3">
                    {(history.lectureViews || []).length +
                      (history.pdfViews || []).length ===
                    0 ? (
                      <div className="text-white/70 text-center p-4">
                        لا توجد مشاهدات لهذا المستخدم.
                      </div>
                    ) : (
                      <>
                        {/* Video views removed - video functionality has been removed from the project */}
                        {history.lectureViews &&
                          history.lectureViews.map((l) => (
                            <div
                              key={`lv-${l._id}`}
                              className="flex items-start gap-3 text-sm text-white/80 border-b border-white/8 py-3"
                            >
                              <button
                                onClick={async () => {
                                  const lectureId =
                                    l.lectureId?._id || l.lectureId;
                                  try {
                                    const res =
                                      await videosAPI.getVideosByLecture(
                                        lectureId,
                                      );
                                    const vids = res.data || [];
                                    if (vids.length === 0) {
                                      toast.info(
                                        "لا يوجد فيديو مرتبط بهذه المحاضرة",
                                      );
                                      return;
                                    }
                  ...
                  <div className="space-y-3">
                    ...
                    history.subscriptions.length === 0 ? (
                      <div className="text-white/70 text-center p-4">
                        لا توجد سجلات اشتراك.
                      </div>
                    ) : (
                      history.subscriptions.map((s) => (
                        <div
                          key={s._id}
                          className="text-sm text-white/80 border-b border-white/8 py-3"
                        >
                          <div className="font-medium text-white">
                            {getSubscriptionLabel(s.type)}
                          </div>
                          <div className="text-xs text-white/60 mt-1">
                            بواسطة: {s.adminId || "نظام"}
                          </div>
            ...
            <div className="mt-6 flex items-center gap-3 justify-end pt-4 border-t border-white/10">
              {editUserId === selectedUser._id ? (
                <>
                  <button
                    onClick={() => handleSaveEdit(selectedUser._id)}
            ...
            <div className="bg-gradient-to-b from-gray-900/95 to-gray-900/95 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="text-white font-semibold">
                  {playingVideo.title || "تشغيل الفيديو"}
                </div>
                <button
                  onClick={() => {
                    setShowVideoPlayer(false);
      ...
      <style jsx="true">{`
        ...
        .animate-fadeIn {
          animation: fadeIn 0.12s cubic-bezier(0.2, 0.9, 0.3, 1) both;
        }
