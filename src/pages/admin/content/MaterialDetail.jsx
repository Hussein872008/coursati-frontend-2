...
import { materialAPI, instructorAPI, chapterAPI, lecturesAPI } from '../../../utils/api';
...
const MaterialDetail = () => {
  useTitle('كورساتي — تفاصيل المادة');
  ...
  const { uploadFile, loading: uploading, error: uploadError } = useFileUpload('/instructors');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setEditData({
        ...
        thumbnail: materialRes.data?.thumbnailUrl || materialRes.data?.thumbnail || ''
      });
      setInstructors(instructorsRes.data || []);
      // load lectures for this material (by fetching chapters for each instructor)
      try {
        setLecturesLoading(true);
        const instrs = instructorsRes.data || [];
        const accumulated = [];
        await Promise.all(instrs.map(async (ins) => {
          try {
            const chRes = await chapterAPI.getChaptersByInstructor(ins._id);
            const chapters = chRes.data || [];
            await Promise.all(chapters.map(async (ch) => {
              try {
                const lRes = await lecturesAPI.getLecturesByChapter(ch._id);
                const ls = lRes.data || [];
                ls.forEach((lv) => accumulated.push({ ...lv, chapterId: ch._id, instructorId: ins._id }));
              } catch (e) {
      ...
      try {
        ...
        const studentsRes = await materialAPI.getStudentsCount(materialId);
        const maybeCount = studentsRes.data && (studentsRes.data.count ?? studentsRes.data);
        setStudentsCount(typeof maybeCount === 'number' ? maybeCount : null);
      } catch (err) {
        setStudentsCount(null);
        setStudentsError(err?.response?.status === 404 ? 'not-implemented' : 'error');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.thumbnail && typeof formData.thumbnail === 'object' && formData.thumbnail instanceof File) {
        ...
        await instructorAPI.createInstructor(formData.title, materialId, formData.thumbnail || null, 0);
        setFormData({ title: '', thumbnail: '' });
        setShowForm(false);
        await loadData();
      }
    } catch (error) {
      // Error creating instructor (handled by UI)
    }
  };

  const handleDeleteInstructor = async (instructorId, instructorName) => {
    if (!window.confirm(`هل أنت متأكد من حذف المدرس "${instructorName}"؟`)) return;
      ...
      <div className="min-h-screen p-6">
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="p-6 rounded-2xl bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 h-80">
    ...
    return (
      <div className="min-h-screen  flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-white/60 mb-6">المادة التي تبحث عنها غير موجودة أو تم حذفها</p>
          <button
  ...
  const breadcrumbs = [
    ...
    { label: material.title, path: `/admin/content/materials/${materialId}` },
  ];
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-6 mb-4 md:mb-0">
              ...
              <div>
                ...
                <p className="text-white/60 text-sm mt-1">المعرف: {material._id.slice(0, 8)}...</p>
              </div>
            </div>
        ...
        {activeTab === 'instructors' && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Instructor Cards - New Design (Right Image, Left Content) */}
              {instructors.map((instructor) => (
                <Link
                  key={instructor._id}
                  to={`/admin/content/materials/${materialId}/instructors/${instructor._id}`}
                  className="group relative"
                  onMouseEnter={() => setHoveredCard(instructor._id)}
                  onMouseLeave={() => setHoveredCard(null)}
                >
                  {/* Glow Effect */}
                  <div className={`absolute inset-0 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 rounded-2xl blur-xl transition-opacity duration-500 ${
                    hoveredCard === instructor._id ? 'opacity-100' : 'opacity-0'
                  }`} />
                  
                  <div className="relative bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 rounded-2xl p-4 md:p-6 hover:border-cyan-500/30 hover:shadow-2xl hover:shadow-cyan-500/10 transition-all duration-500">
                    <div className="flex flex-col md:flex-row gap-6 items-center">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="text-xs text-white/50">#{instructor._id.slice(0, 8)}</span>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm text-white/60">
                            ...
                            <span>المعرف: {instructor._id}</span>
                          </div>
                        </div>
              ...
              {/* Add Instructor Card - Same layout as instructor cards */}
              <div
              ...
              >
                ...
                <div
                  ...
                  className={`relative bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border ${
                    showForm ? 'border-emerald-500/50' : 'border-white/10 group-hover:border-emerald-500/30'
                  } rounded-2xl p-6 cursor-pointer transition-all duration-500 ${
                    !showForm && 'hover:shadow-2xl hover:shadow-emerald-500/10 hover:-translate-y-1'
                  }`}
                >
                  {!showForm ? (
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          <h3 className="text-xl font-bold text-white">إضافة مدرس جديد</h3>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                    <div className="text-sm text-white/50 mb-1">اسم المادة</div>
                    <div className="text-lg font-semibold text-white">{material.title}</div>
                  </div>
                  
                  <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                    <div className="text-sm text-white/50 mb-1">معرف المادة</div>
                    <div className="font-mono text-white/80 break-all">{material._id}</div>
                  </div>
                  
                  <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                    <div className="text-sm text-white/50 mb-1">تاريخ الإنشاء</div>
                    <div className="text-white">
                      {new Date(material.createdAt || Date.now()).toLocaleDateString('ar-SA')}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-xl p-4 border-l-4 border-cyan-400">
                    <div className="text-2xl font-bold text-cyan-300">{instructors.length}</div>
                    <div className="text-sm text-cyan-200">عدد المدرسين</div>
                  </div>
                  
                  <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 rounded-xl p-4 border-l-4 border-emerald-400">
                    <div className="text-2xl font-bold text-emerald-300">{lectures.length}</div>
                    <div className="text-sm text-emerald-200">المحاضرات المضافة</div>
                  </div>
          ...
          <div className="admin-card p-8 bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 rounded-3xl">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-400/20 rounded-2xl p-6 backdrop-blur-sm">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  <div>
                    <h4 className="text-lg font-bold text-red-300 mb-2">منطقة الخطر</h4>
                    <p className="text-red-200/80">حذف هذه المادة سيزيل جميع المدرسين والدروس المرتبطة بها</p>
                  </div>
                  <button
                    onClick={() => {
                      if (window.confirm(`هل أنت متأكد من حذف المادة "${material.title}" وجميع محتوياتها؟`)) {
                        materialAPI.deleteMaterial(materialId)
                          .then(() => navigate('/admin/content/materials'))
                          .catch(err => { /* delete error suppressed in production */ });
                      }
                    }}
              ...
              <div className="bg-gradient-to-r from-amber-500/10 to-yellow-500/10 border border-amber-400/20 rounded-2xl p-6 backdrop-blur-sm">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    <h4 className="text-lg font-bold text-amber-300 mb-1">تعديل المادة</h4>
                    <p className="text-amber-200/80 text-sm">غيّر اسم المادة أو غلافها هنا. المعاينة تحدث فورياً.</p>
                  </div>
                  <div className="flex items-center gap-3">
                    {!editMode ? (
                      <button
                        onClick={() => setEditMode(true)}
                        className="px-6 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-xl hover:shadow-md hover:shadow-amber-500/20 font-semibold"
                      >
                        تعديل
                      </button>
                    ) : (
                      <button
                        ...
                        className="px-6 py-3 bg-white/10 text-white rounded-xl hover:bg-white/20 font-semibold"
                      >
                        إغلاق
                      </button>
                    )}
                ...
                {editSuccess && (
                  <div className="mb-6 inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 rounded-lg backdrop-blur-sm">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="w-48 h-48 rounded-xl overflow-hidden border-2 border-white/10 bg-white/5 flex items-center justify-center">
                      {editData.thumbnail ? (
                        ...
                        <div className="text-center p-4 text-white/40">
                          ...
                          <div className="text-sm">لا يوجد غلاف</div>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-2">
                    {editMode ? (
                      <form
                        onSubmit={async (e) => {
                          e.preventDefault();
                          if (!editData.title || !editData.title.trim()) return;
                          try {
                            setEditSaving(true);
                            ...
                            // Error updating material (handled by UI)
                      ...
                      >
                        <div>
                          <label className="block text-sm font-medium text-white/80 mb-2">اسم المادة</label>
                          <input
                            type="text"
                            required
                            value={editData.title}
                            onChange={(e) => setEditData({ ...editData, title: e.target.value })}
                            className="w-full px-4 py-3 bg-white/5 backdrop-blur-sm border border-white/10 text-white rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-300"
                          />
                        </div>
                        ...
                        <div>
                          <CloudinaryImageInput
                            ...
                            label={<span className="text-sm text-white/80">غلاف المادة (اختياري)</span>}
                          />
                        </div>
                        ...
                        <div className="flex items-center gap-3 pt-2">
                          <button
                            ...
                            className={`px-6 py-3 rounded-xl font-semibold text-white ${
                              editSaving || !editData.title.trim()
                                ? 'bg-emerald-300/50 cursor-not-allowed'
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditData({
                                ...
                                thumbnail: material?.thumbnailUrl || material?.thumbnail || ''
                              });
                            }}
                            className="px-6 py-3 bg-white/10 text-white rounded-xl hover:bg-white/20 font-semibold"
                          >
