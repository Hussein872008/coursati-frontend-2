...
const MaterialDetail = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setEditData({
        ...
        thumbnail:
          materialRes.data?.thumbnailUrl || materialRes.data?.thumbnail || "",
      });
      ...
      try {
        ...
        const maybeCount =
          studentsRes.data && (studentsRes.data.count ?? studentsRes.data);
        setStudentsCount(typeof maybeCount === "number" ? maybeCount : null);
      } catch (err) {
        setStudentsCount(null);
        setStudentsError(
          err?.response?.status === 404 ? "not-implemented" : "error",
        );
      } finally {
  ...
  const handleDeleteInstructor = async (instructorId, instructorName) => {
    if (!window.confirm(`هل أنت متأكد من حذف المدرس "${instructorName}"؟`))
      return;
        ...
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="p-6 rounded-2xl bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 h-80"
              >
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-red-200/80">
                      حذف هذه المادة سيزيل جميع المدرسين والدروس المرتبطة بها
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `هل أنت متأكد من حذف المادة "${material.title}" وجميع محتوياتها؟`,
                        )
                      ) {
                        materialAPI
                          ...
                          .catch((err) => {
                            /* delete error suppressed in production */
                          });
                      }
                    }}
                ...
                <div className="flex items-start justify-between mb-6">
                  <div>
                    ...
                    <p className="text-amber-200/80 text-sm">
                      غيّر اسم المادة أو غلافها هنا. المعاينة تحدث فورياً.
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    {!editMode ? (
