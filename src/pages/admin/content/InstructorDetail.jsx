...
const InstructorDetail = () => {
  ...
  useEffect(() => {
    if (!validateAllNumericIds({ materialId, instructorId }, ['materialId', 'instructorId'])) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setInstructor(instr);
      setEditData({ title: instr.title || '', thumbnail: instr.thumbnail || '' });

      const chData = chaptersRes.data || [];
      const normalized = chData.map((c) => ({ ...c, thumbnail: c.thumbnailUrl || c.thumbnail }));
      setChapters(normalized);
      // load lectures count across all chapters for this instructor
      try {
        if (normalized.length) {
          const lectureLists = await Promise.all(
            normalized.map((ch) => lecturesAPI.getLecturesByChapter(ch._id).then((r) => r.data || []).catch(() => []))
          );
          const total = lectureLists.reduce((sum, arr) => sum + (arr.length || 0), 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.thumbnail && typeof formData.thumbnail === 'object' && formData.thumbnail instanceof File) {
        ...
        await chapterAPI.createChapter(formData.title, instructorId, formData.thumbnail || null, 0);
        setFormData({ title: '', thumbnail: '' });
        setShowForm(false);
        await loadData();
      }
  ...
  const handleDeleteInstructor = async () => {
    if (!window.confirm(`هل أنت متأكد من حذف المدرس "${instructor.title}" وجميع فصوله؟`)) return;
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 p-6">
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="p-6 rounded-2xl bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 h-64">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">المدرس غير موجود</h2>
          <p className="text-white/60 mb-6">المدرس الذي تبحث عنه غير موجود أو تم حذفه</p>
          <button
  ...
  const breadcrumbs = [
    ...
    { label: instructor.title, path: `/admin/content/materials/${materialId}/instructors/${instructorId}` },
  ];
          ...
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="flex items-center gap-6 mb-4 md:mb-0">
              ...
              <div>
                ...
                <p className="text-white/60 text-sm mt-1">المعرف: {instructor._id.slice(0, 8)}...</p>
              </div>
            </div>
                ...
                {chapters.map((chapter) => (
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 rounded-2xl blur-xl transition-opacity duration-500 ${
                      hoveredCard === chapter._id ? 'opacity-100' : 'opacity-0'
                    }`} />
                      ...
                      <div className="flex gap-6 items-start">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-xs text-white/50">#{chapter._id.slice(0, 8)}</span>
                          </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                      <div className="text-sm text-white/50 mb-1">اسم المدرس</div>
                      <div className="text-lg font-semibold text-white">{instructor.title}</div>
                    </div>
                    
                    <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                      <div className="text-sm text-white/50 mb-1">معرف المدرس</div>
                      <div className="font-mono text-white/80 break-all">{instructor._id}</div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-xl p-4 border-l-4 border-cyan-400">
                      <div className="text-2xl font-bold text-cyan-300">{chapters.length}</div>
                    ...
                    <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 rounded-xl p-4 border-l-4 border-emerald-400">
                      <div className="text-2xl font-bold text-emerald-300">{lecturesCount}</div>
                  ...
                  <div className="flex items-start gap-6">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <div>
                          <h4 className="text-lg font-bold text-amber-300">تعديل معلومات المدرّس</h4>
                          <p className="text-sm text-amber-200/80">قم بتعديل اسم المدرّس وصورة العرض</p>
                        </div>
                        <div>
                          {!editMode ? (
                            <button
                            ...
                            <button
                              onClick={() => { 
                                setEditMode(false); 
                                setEditData({ title: instructor.title || '', thumbnail: instructor.thumbnail || '' }); 
                      ...
                      {editMode && (
                        <form onSubmit={async (e) => {
                          ...
                          setEditing(true);
                          try {
                            const res = await instructorAPI.updateInstructor(instructorId, editData.title, editData.thumbnail || null, instructor.order || 0);
                            setInstructor((prev) => ({ ...prev, title: editData.title, thumbnail: editData.thumbnail }));
                            setEditSuccess(true);
                            setEditMode(false);
                          } catch (err) {
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-medium text-white/80 mb-2">اسم المدرّس</label>
                              <input
                                type="text"
                            ...
                            <div>
                              <label className="block text-sm font-medium text-white/80 mb-2">صورة العرض</label>
                              <CloudinaryImageInput
                ...
                <div className="bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-400/20 rounded-2xl p-6 backdrop-blur-sm">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-red-200/80">حذف هذا المدرس سيزيل جميع الفصول والدروس المرتبطة به</p>
                    </div>
                    <button
