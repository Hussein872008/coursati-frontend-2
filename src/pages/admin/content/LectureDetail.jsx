...
const LectureDetailWithMedia = () => {
  ...
  const [videoFormData, setVideoFormData] = useState({ title: '', duration: 0, hours: 0, minutes: 0, seconds: 0, qualities: [{ quality: '360', lastSegmentUrl: '' }] });
  ...
  useEffect(() => {
    if (!validateAllNumericIds({ materialId, instructorId, chapterId, lectureId }, ['materialId', 'instructorId', 'chapterId', 'lectureId'])) {
  ...
  const loadVideos = async () => {
    ...
    try {
      ...
      vids.forEach((vd) => {
        try { fetchVideoViewers(vd._id); } catch (e) {}
      });
    } catch (err) {
  ...
  const loadLecture = async () => {
    ...
    try {
      ...
      setLectureEditThumbnail(res.data?.thumbnailUrl || res.data?.thumbnail || '');
    } catch (error) {
  ...
  const handleLectureSave = async () => {
    try {
      const updated = await lecturesAPI.updateLecture(lecture._id, lectureTitleInput, lectureEditThumbnail || lecture.thumbnailUrl || lecture.thumbnail || '', lecture.order ?? 0);
      setLecture(updated.data || { ...lecture, title: lectureTitleInput, thumbnailUrl: lectureEditThumbnail || lecture.thumbnailUrl || lecture.thumbnail || '' });
  ...
  const handleLectureDelete = async () => {
    setDeleteTarget({ type: 'lecture', id: lecture._id, name: lecture.title || 'المحاضرة' });
  };
  ...
  // derived video stats
  const totalVideoDuration = videos.reduce((sum, v) => sum + (Number(v.duration) || 0), 0);
  const formatHMS = (s) => {
  ...
  const handlePdfSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPdf) {
        await pdfsAPI.updatePdf(editingPdf._id, pdfFormData.title, pdfFormData.url, 0);
        setEditingPdf(null);
      } else {
        await pdfsAPI.createPdf(pdfFormData.title, lectureId, pdfFormData.url, 0);
      }
      resetPdfForm();
      await loadPdfs();
    } catch (error) {
  ...
  const handleVideoSubmit = async (e) => {
    ...
    try {
      ...
      if (editingVideo) {
        ...
        await api.put(`/api/videos/${editingVideo._id}`, payload).catch(async () => {
          await api.put(`/api/admin/videos/${editingVideo._id}`, payload);
        });
        setEditingVideo(null);
      } else {
        await videosAPI.createVideo(payload.title, payload.duration, lectureId, payload.qualities);
        setShowVideoForm(false);
      }

      setVideoFormData({ title: '', duration: 0, hours: 0, minutes: 0, seconds: 0, qualities: [{ quality: '360', lastSegmentUrl: '' }] });
      await loadVideos();
    } catch (err) {
  ...
  const performDelete = async () => {
    ...
    try {
      if (deleteTarget.type === 'lecture') {
        ...
        navigate(`/admin/content/materials/${materialId}/instructors/${instructorId}/chapters/${chapterId}`);
  ...
  const handleEditVideo = (video) => {
    ...
    setVideoFormData({
      ...
      minutes,
      seconds,
      qualities: (video.qualities && video.qualities.length) ? video.qualities : [{ quality: '360', lastSegmentUrl: '' }]
    });
    setShowVideoForm(false);
    setActiveTab('videos');
  };
  ...
  useEffect(() => {
    const handleDocClick = (e) => {
      let el = e.target;
      while (el) {
        if (el.dataset && (el.dataset.viewerPopover === 'true' || el.dataset.viewerButton === 'true')) return;
        el = el.parentElement;
      }
      setActivePdfId(null);
  ...
  const breadcrumbs = [
    ...
    { label: 'الفصول', path: `/admin/content/materials/${materialId}/instructors/${instructorId}` },
    { label: 'المحاضرات', path: `/admin/content/materials/${materialId}/instructors/${instructorId}/chapters/${chapterId}` },
    {
    ...
    return (
      <div className="min-h-screen  flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">المحاضرة غير موجودة</h3>
          <p className="text-white/60 mb-6">المحاضرة التي تبحث عنها غير موجودة أو تم حذفها</p>
          <button
            onClick={() => navigate(`/admin/content/materials/${materialId}/instructors/${instructorId}/chapters/${chapterId}`)}
    ...
    <div className="min-h-screen p-6">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between">
                <div className="flex items-center gap-6 mb-4 md:mb-0">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-sm mt-1">المعرف: {lecture._id.slice(0, 8)}...</p>
                  </div>
                </div>
            ...
            <div className="admin-card p-2 bg-gradient-to-r from-gray-800/40 to-gray-900/40 backdrop-blur-sm border border-white/10 rounded-2xl">
              <div className="flex space-x-2 rtl:space-x-reverse">
                <button
                  ...
                  className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'videos'
                ...
                <button
                  ...
                  className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'pdfs'
                    ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-md'
                    : 'text-white/60 hover:bg-white/5 hover:text-white/80'
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'info'
                    ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-md'
                    : 'text-white/60 hover:bg-white/5 hover:text-white/80'
                    }`}
                >
            ...
            <div className="p-8">
              <AnimatePresence mode="wait">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => { setEditingLectureTitle(true); setLectureTitleInput(lecture.title); setLectureEditThumbnail(lecture.thumbnailUrl || lecture.thumbnail || ''); }}
                        ...
                        <div className="space-y-4">
                          <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                            <div className="text-sm text-white/50 mb-1">اسم المحاضرة</div>
                            <div>
                              {!editingLectureTitle ? (
                                ...
                                <div className="space-y-3">
                                  <div className="flex gap-2 items-center">
                                    ...
                                    />
                                    <button onClick={handleLectureSave} className="px-3 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-lg hover:shadow-lg hover:shadow-cyan-500/20 transition-all duration-200">حفظ</button>
                                    <button onClick={() => setEditingLectureTitle(false)} className="px-3 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-colors">إلغاء</button>
                                  </div>

                                  <div>
                                    <label className="block text-sm text-white/80 mb-2">صورة المحاضرة</label>
                                    <CloudinaryImageInput value={lectureEditThumbnail} onChange={(val) => setLectureEditThumbnail(val)} />
                                  </div>
                                </div>
                              )}
                            </div>
                          ...
                          <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                            <div className="text-sm text-white/50 mb-1">معرف المحاضرة</div>
                            <div className="font-mono text-white/80 break-all">{lecture._id}</div>
                          </div>
                          {lecture.description && (
                            <div className="bg-gradient-to-r from-white/5 to-white/10 rounded-xl p-4 backdrop-blur-sm">
                              <div className="text-sm text-white/50 mb-1">الوصف</div>
                              <div className="text-white/80">{lecture.description}</div>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-4">
                          <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 rounded-xl p-4 border-l-4 border-emerald-400">
                            <div className="text-2xl font-bold text-emerald-300">{pdfs.length}</div>
                            <div className="text-sm text-emerald-200">عدد الملفات</div>
                          </div>
                          <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-xl p-4 border-l-4 border-cyan-400">
                            ...
                            <div className="text-sm text-cyan-200">عدد الفيديوهات</div>
                          </div>
                          <div className="bg-gradient-to-r from-orange-500/10 to-red-500/10 rounded-xl p-4 border-l-4 border-orange-400">
                            <div className="text-2xl font-bold text-orange-300">{formatHMS(totalVideoDuration)}</div>
                            <div className="text-sm text-orange-200">إجمالي مدة الفيديوهات</div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {activeTab === 'videos' && (
                  ...
                  >
                    <div>
                      <h2 className="text-2xl font-bold text-white mb-2">فيديوهات المحاضرة</h2>
                      <p className="text-white/60">أضف أو شغّل فيديوهات مرتبطة بهذه المحاضرة (TS segments)</p>
                    </div>
                    ...
                    {videosLoading ? (
                      <div className="flex items-center justify-center py-12">
                        <div className="text-center">
                          ...
                          <p className="mt-6 text-white/60">جاري تحميل الفيديوهات...</p>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          {videos.map((video, index) => (
                            <React.Fragment key={video._id}>
                              {editingVideo && editingVideo._id === video._id ? (
                                ...
                                >
                                  <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-bold text-white">تعديل الفيديو</h3>
                                    <button onClick={() => setEditingVideo(null)} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                                      <X className="w-5 h-5 text-white/80" />
                                    </button>
                                  </div>
                                  <form onSubmit={handleVideoSubmit} className="space-y-4">
                                    <div>
                                      <label className="block text-sm font-medium text-white/80 mb-2">عنوان الفيديو</label>
                                      <input
                                        required
                                    ...
                                    <div>
                                      <label className="block text-sm font-medium text-white/80 mb-2">المدة (ساعة:دقيقة:ثانية)</label>
                                      <div className="grid grid-cols-3 gap-2">
                                        <input
                                          type="number"
                                          min={0}
                                          value={videoFormData.hours}
                                          onChange={(e) => setVideoFormData({ ...videoFormData, hours: e.target.value })}
                                          className="w-full px-3 py-2 bg-white/5 backdrop-blur-sm border border-white/10 text-white rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-300"
                                          placeholder="ساعات"
                                        />
                                        <input
                                          ...
                                          onChange={(e) => setVideoFormData({ ...videoFormData, minutes: e.target.value })}
                                          className="w-full px-3 py-2 bg-white/5 backdrop-blur-sm border border-white/10 text-white rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-300"
                                          placeholder="دقائق"
                                        />
                                        <input
                                          ...
                                          onChange={(e) => setVideoFormData({ ...videoFormData, seconds: e.target.value })}
                                          className="w-full px-3 py-2 bg-white/5 backdrop-blur-sm border border-white/10 text-white rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-300"
                                          placeholder="ثواني"
                                        />
                                      </div>
                                      <div className="mt-2 text-sm text-white/60">المجموع: {Math.floor((Number(videoFormData.hours)||0)*3600 + (Number(videoFormData.minutes)||0)*60 + (Number(videoFormData.seconds)||0))} ثانية</div>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-white/80 mb-2">الجودات</label>
                                      {videoFormData.qualities.map((q, idx) => (
                                        <div key={idx} className="grid grid-cols-3 gap-2 mb-2">
                                          <input
                                            value={q.quality}
                                            onChange={(e) => {
                                              const arr = [...videoFormData.qualities];
                                              arr[idx].quality = e.target.value;
                                              setVideoFormData({ ...videoFormData, qualities: arr });
                                            }}
                                          ...
                                          <input
                                            ...
                                            onChange={(e) => {
                                              const arr = [...videoFormData.qualities];
                                              arr[idx].lastSegmentUrl = e.target.value;
                                              setVideoFormData({ ...videoFormData, qualities: arr });
                                            }}
                                      ...
                                      <div>
                                        <button
                                          ...
                                          onClick={() => setVideoFormData({
                                            ...videoFormData,
                                            qualities: [...videoFormData.qualities, { quality: '720', lastSegmentUrl: '' }]
                                          })}
                                  ...
                                  <div className="flex gap-6 items-center">
                                    ...
                                    <div className="flex-1">
                                      <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="text-xs text-white/50">#{video._id.slice(0, 8)}</span>
                                      </div>
                                      ...
                                      <div className="space-y-2">
                                        <div className="flex items-center gap-2 text-sm text-white/60">
                                          ...
                                          <span>المدة: {video.duration ? formatHMS(video.duration) : 'غير محدد'}</span>
                                        </div>

                                        <div className="flex items-center gap-2 text-sm text-white/60">
                                          <div className="flex items-center gap-2">
                                            <BarChart3 className="w-4 h-4" />
                                            <span>الجودات: {video.qualities.map((q) => q.quality).join(' ، ')}</span>
                                          </div>
                                        </div>
                                      </div>
                                      ...
                                      <div className="mt-6 flex items-center gap-4">
                                        ...
                                        <div className="ml-2 relative">
                                          <button
                                            ...
                                            onClick={(e) => {
                                              ...
                                              if (videoPopover && videoPopover.videoId === video._id) {
                                                setVideoPopover(null);
                                              } else {
                                                setVideoPopover({ videoId: video._id, left: rect.left, top: rect.bottom, width: rect.width });
                                          ...
                                          >
                                            ...
                                            <span>{(videoViewersMap[video._id] || []).length}</span>
                                          </button>
                                        </div>
                                        ...
                                        <div className="ml-2 relative">
                                          <button
                                            ...
                                            onClick={(e) => {
                                              const rect = e.currentTarget.getBoundingClientRect();
                                              if (videoPopover && videoPopover.videoId === `dl-${video._id}`) {
                                                setVideoPopover(null);
                                              } else {
                                                setVideoPopover({ videoId: `dl-${video._id}`, left: rect.left, top: rect.bottom, width: rect.width });
                                          ...
                                          >
                                            ...
                                            <span>{(videoDownloadsMap[video._id] || []).length}</span>
                                          </button>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 ml-auto">
                                          <button
                                            onClick={() => {
                                              videosAPI.validateVideo(video._id, false)
                                                .then(res => { alert('تم التحقق من الفيديو'); })
                                                .catch(err => { alert('فشل التحقق من الفيديو'); });
                                            }}
                          ...
                          {showVideoForm ? (
                            ...
                            >
                              <div className="flex items-center justify-between mb-6">
                                <h3 className="text-lg font-bold text-white">إضافة فيديو جديد</h3>
                                <button onClick={() => setShowVideoForm(false)} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                                  <X className="w-5 h-5 text-white/80" />
                                </button>
                              </div>
                              <form onSubmit={handleVideoSubmit} className="space-y-4">
                            ...
                            <div
                              onClick={() => { setShowVideoForm(true); setVideoFormData({ title: '', duration: 0, hours: 0, minutes: 0, seconds: 0, qualities: [{ quality: '360', lastSegmentUrl: '' }] }); }}
                ...
                {activeTab === 'pdfs' && (
                  ...
                  >
                    <div>
                      <h2 className="text-2xl font-bold text-white mb-2">ملفات المحاضرة</h2>
                      <p className="text-white/60">إدارة وعرض جميع ملفات PDF للمحاضرة</p>
                    </div>
                    ...
                    {pdfsLoading ? (
                      <div className="flex items-center justify-center py-12">
                        <div className="text-center">
                          <div className="w-16 h-16 border-4 border-white/10 border-t-emerald-400 rounded-full animate-spin mx-auto"></div>
                          <p className="mt-6 text-white/60">جاري تحميل الملفات...</p>
                        </div>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {pdfs.map((pdf, index) => (
                          <React.Fragment key={pdf._id}>
                            {editingPdf && editingPdf._id === pdf._id ? (
                              ...
                              >
                                <div className="flex items-center justify-between mb-6">
                                  <h3 className="text-lg font-bold text-white">تعديل الملف</h3>
                                  <button onClick={resetPdfForm} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                                    <X className="w-5 h-5 text-white/80" />
                                  </button>
                                </div>
                                <form onSubmit={handlePdfSubmit} className="space-y-4">
                                  <div>
                                    <label className="block text-sm font-medium text-white/80 mb-2">عنوان الملف</label>
                                    <input
                                      type="text"
                                      required
                                  ...
                                  <div>
                                    <PDFUpload
                                      ...
                                      onChange={(urlOrFile) => setPdfFormData({ ...pdfFormData, url: urlOrFile })}
                                      label="رفع ملف PDF"
                                      autoUpload={false}
                                    />
                                  </div>
                                  <div className="flex justify-end gap-3">
                                    <button type="button" onClick={resetPdfForm} className="px-4 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-colors">إلغاء</button>
                                    <button type="submit" disabled={isUploadingPdf} className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg hover:shadow-lg hover:shadow-emerald-500/20 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
                                ...
                                <div className="flex gap-6 items-center">
                                  ...
                                  <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-4">
                                      ...
                                      <span className="text-xs text-white/50">#{pdf._id.slice(0, 8)}</span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="flex items-center gap-2 text-sm text-white/60">
                                        <button
                                          ...
                                          onClick={(e) => {
                                            const rect = e.currentTarget.getBoundingClientRect();
                                            if (pdfPopover && pdfPopover.pdfId === pdf._id) {
                                              setPdfPopover(null);
                                              setActivePdfId(null);
                                            } else {
                                              setPdfPopover({ pdfId: pdf._id, left: rect.left, top: rect.bottom, width: rect.width });
                                        ...
                                        >
                                          <Eye className="w-4 h-4 text-emerald-300" />
                                          <span className="whitespace-nowrap">{pdf.viewCount || 0} مشاهدات</span>
                                        </button>
                                      </div>
                                    </div>
                                        ...
                                        onClick={(ev) => {
                                          ...
                                          try {
                                            ...
                                            if (navigator && typeof navigator.sendBeacon === 'function') {
                                              ...
                                              fetch(viewUrl, { method: 'POST', keepalive: true, headers: { 'Content-Type': 'application/json', 'user-code': localStorage.getItem('userCode') || '' } }).catch(() => { });
                                            }
                                          } catch (err) {
                                            // viewPdf call failed (silenced)
                                          }
                                          if (pdf.fileUrl || pdf.url) window.open(pdf.fileUrl || pdf.url, '_blank');
                                        }}
                        ...
                        {showPdfForm && !editingPdf ? (
                          ...
                          >
                            <div className="flex items-center justify-between mb-6">
                              <h3 className="text-lg font-bold text-white">إضافة ملف جديد</h3>
                              <button onClick={resetPdfForm} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                                <X className="w-5 h-5 text-white/80" />
                              </button>
                            </div>
                          ...
                          !editingPdf && (
                            <div
                              onClick={() => { setShowPdfForm(true); setPdfFormData({ title: '', url: '' }); }}
      ...
      <AnimatePresence>
        {showVideoModal && selectedVideo && (
          ...
          >
            <div className="absolute inset-0 bg-black/60" onClick={() => { setShowVideoModal(false); setSelectedVideo(null); }} />
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }} className="relative w-full max-w-4xl mx-4">
              <div className="p-4">
                <div className="bg-transparent rounded-lg">
                  <div className="flex items-start justify-end mb-4">
                    <button onClick={() => { setShowVideoModal(false); setSelectedVideo(null); }} className="p-2 bg-white/10 hover:bg-white/20 rounded-md text-white">
                      إغلاق
                    </button>
                  </div>
          ...
          >
            ...
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }} className="relative w-full max-w-lg mx-4">
              <div className="bg-gradient-to-r from-gray-800/80 to-gray-900/80 border border-white/10 rounded-2xl p-6">
                ...
                <p className="text-white/80 mb-6">هل أنت متأكد من حذف {deleteTarget?.name}؟ هذا الإجراء لا يمكن التراجع عنه.</p>
                <div className="flex justify-end gap-3">
                  <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20">إلغاء</button>
                  <button onClick={performDelete} disabled={isDeleting} className="px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg disabled:opacity-60">
      ...
      {videoPopover && (
        <div style={{ position: 'fixed', left: videoPopover.left, top: videoPopover.top, zIndex: 9999, transform: 'translateX(-80px)' }}>
          <div data-viewer-popover="true" className="w-64 bg-gray-900/95 text-white text-sm rounded-lg p-3 border border-white/10 shadow-lg">
            <div className="font-semibold text-white/90 mb-2">مشاهدات الفيديو ({(videoViewersMap[videoPopover.videoId] || []).length})</div>
              ...
              <div className="space-y-2 viewer-scroll">
                {(videoViewersMap[videoPopover.videoId] || []).map((v) => (
                  <div key={v._id} className="flex items-start justify-between">
                    <div className="text-white/90">{v.userId?.name || v.userName || 'مستخدم'}</div>
                    <div className="text-white/60 text-xs">{new Date(v.createdAt).toLocaleString()}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      {pdfPopover && (
        <div style={{ position: 'fixed', left: pdfPopover.left, top: pdfPopover.top, zIndex: 9999, transform: 'translateX(-80px)' }}>
          <div data-viewer-popover="true" className="w-64 bg-gray-900/95 text-white text-sm rounded-lg p-3 border border-white/10 shadow-lg">
            <div className="font-semibold text-white/90 mb-2">مشاهدات الملف ({(pdfViewersMap[pdfPopover.pdfId] || []).length})</div>
            {pdfViewersLoadingId === pdfPopover.pdfId ? (
              ...
              <div className="space-y-2 viewer-scroll">
                {(pdfViewersMap[pdfPopover.pdfId] || []).map((v) => (
                  <div key={v._id || v.user?._id || v.userId} className="text-sm text-white/80 px-2 py-1 border-b border-gray-700/50 last:border-0">
                    <div className="font-medium">{(v.name || v.code || v.user?.name || v.user?.code || v.userCode || v.user?.phone || 'مستخدم')}</div>
                    <div className="text-xs text-white/50">{new Date(v.createdAt || v.viewedAt || v.created_at || v.viewed_at).toLocaleString('ar-SA')}</div>
                  </div>
                ))}
              </div>
            )}
