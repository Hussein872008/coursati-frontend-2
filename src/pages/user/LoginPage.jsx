  ...
  useEffect(() => {
    try {
      ...
      if (params.get('expired') === '1') {
        ...
        setCodeWarning(
          <a href={waLink} target="_blank" rel="noopener noreferrer" className="text-amber-200 underline">
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      try {
        ...
        if (status === 403) {
          setError(messageFromServer || 'تم رفض الدخول: الحساب مرتبط بمتصفح آخر');
          setShake(true);
          setTimeout(() => setShake(false), 500);
          setLoading(false);
          return;
      ...
      try { toast.success('تم تسجيل الدخول بنجاح'); } catch (e) { /* ignore if toast unavailable */ }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center" dir="rtl">
      ...
      <div className="relative z-10 w-full max-w-sm sm:max-w-md px-4 mx-auto mt-6 sm:mt-0">
        ...
        <div className={`relative bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl p-8 transform transition-all duration-700 `}>
          {/* حدود إضاءة */}
          ...
          <div className="text-center mb-10 relative">
            ...
            <p className="text-lg text-cyan-200 font-medium tracking-wide">نظام إدارة التعلم الذكي</p>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full transition-all duration-300 ${code.length === 9 ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`} />
                    <span className={`text-sm font-medium transition-all duration-300 ${code.length === 9 ? 'text-green-400' : 'text-gray-400'}`}>
                      {code.length === 9 ? 'الكود مكتمل' : 'أدخل 9 أرقام'}
                    </span>
                  </div>
                  <span className={`text-lg font-bold px-3 py-1 rounded-lg transition-all duration-300 ${code.length === 9 ? 'bg-green-500/20 text-green-400' : 'bg-gray-800 text-gray-400'}`}>
                    {code.length} / 9
                  </span>
                </div>
              </div>
          ...
          <div className="mt-10 pt-8 border-t border-white/10">
            <div className="text-center text-gray-300 mb-6">
              <p className="text-sm text-gray-400">يمكنك التواصل معنا للحصول على كود اشتراك والوصول إلى جميع المحتويات المميزة</p>
            </div>
