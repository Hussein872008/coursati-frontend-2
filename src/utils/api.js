...
export const authAPI = {
  ...
  createUser: (name, phone, subscriptionType) => api.post('/auth/create-user', { name, phone, subscriptionType }),
  ...
  updateUser: (id, name, phone, canDownloadVideos) => {
    ...
    if (typeof canDownloadVideos !== 'undefined') body.canDownloadVideos = !!canDownloadVideos;
    return api.put(`/auth/users/${id}`, body);
  },
  updateUserSubscription: (id, type) => api.put(`/auth/users/${id}/subscription`, { type }),
...
export const treeAPI = {
  getContentTree: () => api.get('/api/tree'),
};
...
export const instructorAPI = {
  createInstructor: (title, materialId, thumbnailUrl, order) => {
    ...
    if (thumbnailUrl && typeof File !== 'undefined' && thumbnailUrl instanceof File) {
      ...
      return api.post('/api/instructors', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
    return api.post('/api/instructors', { title, materialId, thumbnailUrl, order });
  },
  ...
  updateInstructor: (id, title, thumbnailUrl, order) => {
    if (thumbnailUrl && typeof File !== 'undefined' && thumbnailUrl instanceof File) {
      ...
      fd.append('title', title);
      fd.append('order', order ?? 0);
      return api.put(`/api/instructors/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
...
export const chapterAPI = {
  createChapter: (title, instructorId, thumbnailUrl, order) => {
    ...
    if (thumbnailUrl && typeof File !== 'undefined' && thumbnailUrl instanceof File) {
      ...
      fd.append('order', order ?? 0);
      return api.post('/api/chapters', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
    return api.post('/api/chapters', { title, instructorId, thumbnailUrl, order });
  },
  ...
  updateChapter: (id, title, thumbnailUrl, order) => {
    if (thumbnailUrl && typeof File !== 'undefined' && thumbnailUrl instanceof File) {
      ...
      fd.append('title', title);
      fd.append('order', order ?? 0);
      return api.put(`/api/chapters/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
...
export const lecturesAPI = {
  createLecture: (title, chapterId, thumbnailUrl, order) => {
    ...
    if (thumbnailUrl && typeof File !== 'undefined' && thumbnailUrl instanceof File) {
      ...
      fd.append('order', order ?? 0);
      return api.post('/api/lectures', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    }
...
export const adminAPI = {
  ...
  getTimeSeries: (days = 30) => api.get(`/api/admin/stats/timeseries?days=${days}`),
};
  ...
  createPdf: (title, lectureId, fileUrl, order) =>
    ...
    (fileUrl && typeof File !== 'undefined' && fileUrl instanceof File)
      ? (() => {
          ...
          fd.append('order', order ?? 0);
          return api.post('/api/pdfs', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        })()
  ...
  updatePdf: (id, title, fileUrl, order) =>
    (fileUrl && typeof File !== 'undefined' && fileUrl instanceof File)
      ? (() => {
          ...
          fd.append('title', title);
          fd.append('order', order ?? 0);
          return api.put(`/api/pdfs/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        })()
...
export const videosAPI = {
  createVideo: (title, duration, lectureId, qualities) =>
    ...
    api.post(`/api/admin/lectures/${lectureId}/videos`, { title, duration, qualities }),
  getVideosByLecture: (lectureId) => api.get(`/api/videos/lecture/${lectureId}`),
  ...
  signSegment: (videoId, quality, segmentNumber) => api.post(`/api/videos/${videoId}/sign`, { quality, segmentNumber }),
  validateVideo: (videoId, mirror = false) => api.post(`/api/admin/videos/${videoId}/validate`, { mirror }),
};
